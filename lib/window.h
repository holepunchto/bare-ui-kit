...
bare_ui_kit_window_init(js_env_t *env, js_callback_info_t *info) {
  ...
  @autoreleasepool {
    BareWindow *handle = [[[BareWindow alloc] init] autorelease];
...
bare_ui_kit_window_root_view_controller(js_env_t *env, js_callback_info_t *info) {
  ...
  @autoreleasepool {
    ...
    if (argc == 1) {
      ...
      err = js_create_external(env, (void *) CFBridgingRetain(view_controller), bare_ui_kit__on_bridged_release, NULL, &result);
      assert(err == 0);
    } else {
      void *handle;
